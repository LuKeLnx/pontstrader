...
#        break
    ...
    try:
      ...
      if satorper == 1:
        try:
          stoploss = raw_input(Fore.WHITE+'Stop Loss? [eg. 0.00436] : ')
        ...
        try:
          stoploss = raw_input(Fore.WHITE+'Stop Loss percentage (without %)? [eg. 5] : ')
          stoploss = float(stoploss)
        except:
          white('\nInvalid number... going back to Main Menu')
          time.sleep(1)
...
#      yellow('Pontstrader wil calculate new targets and stop loss based on the buyprice.')
      ...
      if satorper == 1:
        ...
        try:
          target = raw_input(Fore.WHITE+'Target in percentage (without %)?  [eg. 5] : ')
          target = float(target)
        except:
          white('\nInvalid number... going back to Main Menu')
          time.sleep(1)
      ...
      if satorper == 1:
        ...
        yellow('- Stop Loss:              {0:.8f} ({1:.2f}%)'.format(float(stoploss), float(stop_per)))
        yellow('- Target:                 {0:.8f} ({1:.2f}%)'.format(float(target), float(tar_per)))
      white((40 * '-'))
